...
EnegyMeter::EnegyMeter(lv_obj_t *parent) : val(0), min_val(0), max_val(100), threshold1(50), threshold2(80), increasing(true) {
...
void EnegyMeter::timer_cb(lv_timer_t *timer) {
    EnegyMeter *meter = (EnegyMeter *)timer->user_data;
    meter->update();
}

void EnegyMeter::setRange(int32_t min, int32_t max) {
    min_val = min;
    max_val = max;
...
void EnegyMeter::setThresholds(int32_t val1, int32_t val2) {
    threshold1 = val1;
    threshold2 = val2;
